...
with tab2:
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.download_button("Download as CSV", data=convert_to_csv(pd.DataFrame([export_data])), file_name="department_efficiency.csv", mime="text/csv")
        st.download_button("Download as JSON", data=convert_to_json(export_data), file_name="department_efficiency.json", mime="application/json")
    with col2:
        st.download_button("Download as XML", data=convert_to_xml(export_data), file_name="department_efficiency.xml", mime="application/xml")
        st.download_button("Download as PDF", data=convert_to_pdf(export_data), file_name="department_efficiency.pdf", mime="application/pdf")
